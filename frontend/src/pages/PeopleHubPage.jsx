      ...
      <div className="flex gap-1 bg-stone-100 rounded-xl p-1 w-fit flex-wrap">
        {[['events', 'Events'], ['awards', 'Awards'], ['csr', 'CSR'], ['queries', 'Appraisal Queries']].map(([k, l]) => (
          <button key={k} onClick={() => setTab(k)}
            className={`text-sm font-medium px-3 py-1.5 rounded-lg ${tab === k ? 'bg-white shadow-sm' : 'text-slate-500'}`}>{l}</button>
        ))}
      </div>
      {tab === 'events' && <Events />}
...
function Events() {
  ...
  return (
    <div className="card divide-y divide-stone-100">
      {rows.map(e => (
        <div key={e.id} className="p-3 flex flex-wrap items-center gap-2">
          <div className="flex-1"><p className="text-sm font-semibold">{e.title}</p>
            <p className="text-xs text-slate-400">{new Date(e.starts_at).toLocaleString('en-IN')} {e.location && `· ${e.location}`} · {e.yes_count} attending</p></div>
          {['yes', 'maybe', 'no'].map(r => (
      ...
      {!rows.length && <p className="p-6 text-center text-sm text-slate-400">No events scheduled.</p>}
    </div>
  );
}
...
function Awards({ user }) {
  ...
  return (
    <div className="card divide-y divide-stone-100">
      {data.cycles.map(c => (
        <div key={c.id} className="p-3 flex flex-wrap items-center gap-2">
          <div className="flex-1"><p className="text-sm font-semibold">{c.program_name} — {c.name}</p>
            <p className="text-xs text-slate-400">{c.nominations} nomination(s)</p></div>
          <span className={`chip ${c.status === 'open' ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-slate-600'}`}>{c.status}</span>
      ...
      {!data.cycles.length && <p className="p-6 text-center text-sm text-slate-400">No award cycles.</p>}
    </div>
  );
}
...
function Csr() {
  ...
  return (
    <div className="card divide-y divide-stone-100">
      {rows.map(c => (
        <div key={c.id} className="p-3 flex flex-wrap items-center gap-2">
          <div className="flex-1"><p className="text-sm font-semibold">{c.title}</p>
            <p className="text-xs text-slate-400">{c.event_date || 'date TBD'} · {c.participants} participating {c.my_hours != null && `· you: ${c.my_hours}h`}</p></div>
      ...
      {!rows.length && <p className="p-6 text-center text-sm text-slate-400">No CSR activities.</p>}
    </div>
  );
}
        ...
        {data.queries.map(q => (
          <div key={q.id} className="p-3">
            <button className="w-full text-left flex items-center gap-2" onClick={() => setOpenQ(v => v === q.id ? null : q.id)}>
              ...
              {data.admin && <span className="text-xs text-slate-400">{q.employee_name}</span>}
              <span className={`chip ${q.status === 'open' ? 'bg-amber-100 text-amber-700' : q.status === 'answered' ? 'bg-blue-100 text-blue-700' : 'bg-stone-100 text-slate-600'}`}>{q.status}</span>
            </button>
            {openQ === q.id && <Thread id={q.id} reload={load} />}
          </div>
        ))}
        {!data.queries.length && <p className="p-6 text-center text-sm text-slate-400">No queries.</p>}
      </div>
    </div>
  );
}
