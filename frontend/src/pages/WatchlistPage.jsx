  ...
  return (
    <div className="space-y-4 max-w-4xl">
      <div>
        ...
        <p className="text-xs text-slate-400">Employees with 3 consecutive top-tier ratings, most recently rated the highest grade. Recomputed automatically each time a cycle publishes; a lapsed streak removes someone from this list.</p>
      </div>
      {err && <p className="text-sm text-rose-600">{err}</p>}
      {!err && !rows && <p className="text-sm text-slate-400">Loading…</p>}
      {rows && !rows.length && <div className="card p-8 text-center text-sm text-slate-400">No one currently qualifies — this list updates as cycles publish.</div>}
      {rows && rows.length > 0 && (
