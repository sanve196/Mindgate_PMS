...
export default function ConnectsPage() {
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      {!data.length && <div className="card p-8 text-center text-sm text-slate-400">No connects logged yet.</div>}
  ...
  return (
    <div className="card p-4 space-y-2">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold">{cn.employee_name} <span className="text-slate-400 font-normal">with {cn.manager_name}</span></p>
          <p className="text-xs text-slate-400">{new Date(cn.held_at).toLocaleDateString()}</p>
        </div>
      ...
      {cn.notes && <p className="text-xs text-slate-600">{cn.notes}</p>}
      <div className="flex gap-2">
