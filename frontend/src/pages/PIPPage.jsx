...
const STATUS_COLOR = {
  ...
  closed_unsuccessful: 'bg-slate-200 text-slate-600',
};
...
export default function PIPPage() {
  ...
  if (!pips) return <p className="text-sm text-slate-400">Loading…</p>;

  return (
    <div className="space-y-4 max-w-4xl">
      <div>
        <h2 className="text-lg font-bold">Performance Improvement Plans</h2>
        <p className="text-xs text-slate-400">Auto-opened when a published rating falls below the cycle's threshold. Weekly notes are added by the manager or HR; the employee has read-only visibility.</p>
      </div>
      {!pips.length && <div className="card p-8 text-center text-sm text-slate-400">No PIPs — either none has been triggered, or you have none to view.</div>}
      <div className="space-y-2">
        {pips.map(p => (
          <div key={p.id} className="card">
            <button className="w-full flex items-center justify-between p-4 text-left" onClick={() => setOpenId(openId === p.id ? null : p.id)}>
              <div>
                <p className="font-semibold text-sm">{p.employee_name} <span className="text-slate-400 font-normal">· {p.cycle_name || '—'}</span></p>
                <p className="text-xs text-slate-400">Opened {new Date(p.opened_at).toLocaleDateString()} · {p.opened_by}</p>
              </div>
...
function PIPDetail({ id, onChange }) {
  ...
  useEffect(() => { load(); }, [id]);
  if (!data) return <p className="px-4 pb-4 text-xs text-slate-400">Loading…</p>;
  ...
  return (
    <div className="px-4 pb-4 space-y-3 border-t border-stone-100 pt-3">
      ...
      {pip.closed_reason && <p className="text-xs bg-stone-50 rounded-lg p-2"><span className="font-semibold">Closure note:</span> {pip.closed_reason}</p>}

      <div>
        <p className="lbl mb-1">Weekly notes</p>
        {!weekly_entries.length && <p className="text-xs text-slate-400">No weekly notes yet.</p>}
        <div className="space-y-1">
          {weekly_entries.map(w => (
            <div key={w.id} className="text-xs bg-stone-50 rounded-lg p-2">
              <p className="font-semibold">Week ending {new Date(w.week_ending).toLocaleDateString()} <span className="text-slate-400 font-normal">· {w.submitted_by}</span></p>
